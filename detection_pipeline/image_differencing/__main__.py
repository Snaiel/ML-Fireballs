...
from .difference import difference_images
...
def main() -> None:
    ...
    print("Transformation Magnitude:", transformation_magnitude)
    ...
    fig_diff, axs_diff = plt.subplots(2, 2, figsize=(8, 8), sharex=True, sharey=True)

    axs_diff[0, 0].imshow(image2, cmap='gray')
    axs_diff[0, 0].set_title("Original Image 2")
    axs_diff[0, 0].axis("off")

    axs_diff[0, 1].imshow(difference_images(image2, image1), cmap='gray')
    axs_diff[0, 1].set_title("difference_images Output")
    axs_diff[0, 1].axis("off")

    axs_diff[1, 0].imshow(original_difference, cmap='gray')
    axs_diff[1, 0].set_title("Unaligned Blurred Difference")
    axs_diff[1, 0].axis("off")

    axs_diff[1, 1].imshow(blurred_difference, cmap='gray')
    axs_diff[1, 1].set_title("Aligned Blurred Difference")
    axs_diff[1, 1].axis("off")
