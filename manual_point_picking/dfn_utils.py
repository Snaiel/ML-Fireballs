...
def get_time_from_trajectory_stuby_file(fname):
    # parse a filename like 2021-05-01T19-04-58.0_DFNSMALL00_DFNSMALL00_trajectory.csv
    f = os.path.basename(fname)
    datetime_block = f.split('_')[0]
    dateblock = datetime_block.split('T')[0]
    timeblock = datetime_block.split('T')[1]
    
    time = timeblock.replace('-',':')
    
    ts = Time(dateblock + 'T' + time)
    
    return ts

def get_time_from_video_detection(fname):
    
    # DFNEXT010_detection_2021-05-04_202320
    f = os.path.basename(fname)
    dateblock = f.split('_')[2]
    timeblock = f.split('_')[3]
    time = timeblock[:2] + ':' + timeblock[2:4] + ':' + timeblock[4:6]
...
def find_log_file(basedir, suffix, extension='txt', system_number=''):
    ...
    if len(list_results) < 1:
        raise FileNotFoundError('Could not locate log file using the command: ', find_comm)
